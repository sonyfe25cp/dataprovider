		...
		for(int index = 0 ; index < trs.size(); index ++){//保证顺序读取
			...
			if(tds.size() == 1 && spans.size() == 1){
				...
				if(rawText.contains("[") && rawText.contains("]")){
					String range = rawText.substring(rawText.indexOf("[")+1, rawText.indexOf("]"));
					range = range.replace(" ", "");
					...
					if(tmpArray.length == 2){
						...
						if(company.contains("(") && company.contains(")")){
							String size = company.substring(company.lastIndexOf("(")+1, company.lastIndexOf(")"));
	...
	private void trainExpriencesDetect(Element table){
		...
		for(int index = 0 ; index < trs.size(); index ++){//保证顺序读取
			...
			if(tds.size() == 4){
				...
				if(kecheng.length() > 0)
					subJsonArray.add(json("培训课程", kecheng));
				if(zhengshu.length() > 0)
					subJsonArray.add(json("培训证书", zhengshu));
			}else if(tds.size() == 1){
				String content = tds.text().replace(" ", "");
				if(content.length() > 0)
					subJsonArray.add(json("培训内容", content));
			}
		}
		jsonArray.add(subJsonArray);
		String partJson = json(jsonArray, categoryName);
		...
		for(Element tr : table.select("tr")){
			...
			if(rawText.contains("（") && rawText.contains("）")){
				...
				if(languageLevel.contains("（") && languageLevel.contains("）")){
					...
					String level = languageLevel.substring(languageLevel.indexOf("（")+1, languageLevel.indexOf("）"));
				...
				if(detailsLevel.contains("，")){
					...
					for(String group : groups){
						if(group.contains("（") && group.contains("）")){
							...
							String level = group.substring(group.indexOf("（")+1, group.indexOf("）"));
	...
	private void papersDetect(Element table){
		...
		for(Element tr : table.select("tr")){
			...
			if(tds.size() == 3){
				...
				if(last.length() > 0)
					subLanguage.add(json("证书等级", last));
				languageArray.add(subLanguage);
			}
		}
		String partJson = json(languageArray, categoryName);
	...
	private void scoresDetect(Element table){
		...
		for(Element tr : table.select("tr")){
			...
			if(tds.size() == 3){
				...
				if(desc.length() > 0)
					subLanguage.add(json("奖项说明", desc));
				languageArray.add(subLanguage);
			}
		}
		String partJson = json(languageArray, categoryName);
